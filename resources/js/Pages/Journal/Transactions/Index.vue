<template>
  <div class="py-10 mx-auto max-w-7xl sm:px-6 lg:px-8">
    ...
    >
      ...
      >
        <JetButton @click="toggleShowAdd(true, true)" class="w-32">
          Add Income
        </JetButton>
        <JetButton @click="toggleShowAdd(false, true)" class="w-36">
          Add Expense
        </JetButton>
      </div>
    </div>
    ...
    <div class="w-full">
      ...
      >
        ...
        <template v-slot:account="{ scope: { row } }">
          <div class="font-bold">{{ row.account ? row.account.name : "" }}</div>
        </template>
