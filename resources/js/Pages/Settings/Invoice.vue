...
const formData = reactive({
  ...
  invoice_hide_company_logo: Boolean(props.settingData.invoice_hide_company_logo),
});
    ...
    <template #header>
      <div class="flex items-center justify-between px-5 py-2 rounded-md">
        ...
        <div class="flex overflow-hidden font-bold text-gray-500 rounded-t-lg max-w-min">
          <AppButton variant="inverse" @click="save()"> {{ $t("save") }} </AppButton>
        </div>
      </div>
    </template>
    ...
    >
      ...
      <article>
        ...
        <AtField class="md:max-w-sm" label="Factura Compra" rounded>
          <AtInput
            v-model="formData.invoice_purchase_label"
            placeholder="Factura Compra"
            rounded
          />
        </AtField>
        <AtField class="md:max-w-sm" label="Ocultar logo en factura" rounded>
          <AtFieldCheck
            v-model="formData.invoice_hide_company_logo"
            label="Ocultar logo?"
            rounded
          />
        </AtField>
