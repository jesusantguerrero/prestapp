...
const onSubmit = () => {
  ElMessageBox.alert("¿Seguro que desea terminar este contrato?", "Terminar Contrato", {
    ...
    callback: (action: Action) => {
      if (action === "confirm") {
        endRentForm.put(
          route("tenant.end-rent-action", { client: props.clients, rent: props.rent })
        );
      }
    },
  });
    ...
    <main class="w-full px-4 py-2 space-y-4 rounded-md shadow-md bg-base-lvl-3">
      ...
      <section>
        ...
        <section class="space-y-2">
          <InvoiceCard v-for="invoice in pendingInvoices" :invoice="invoice" />
          <span class="text-body-1" v-if="!pendingInvoices.length">
            No hay facturas pendientes
          </span>
        </section>
        <p class="text-end">
          <AtButton class="font-bold text-success"> Agregar Factura </AtButton>
        </p>
      </section>
      ...
      <section>
        ...
        <section>
          <InvoiceCard v-for="invoice in depositsToReturn" :invoice="invoice" />
          <span class="text-body-1" v-if="!depositsToReturn.length">
            No hay depositos pendientes
          </span>
        </section>
        <p class="text-end">
          <AtButton v-if="depositsToReturn.length" class="font-bold text-success"
            >Retornar deposito</AtButton
          >
        </p>
      ...
      <footer class="flex justify-between">
        ...
        <section class="flex space-x-2">
          ...
          <AppButton variant="error" @click="onSubmit"> Finalizar Contrato </AppButton>
        </section>
      </footer>
    </main>
  </ClientTemplate>
