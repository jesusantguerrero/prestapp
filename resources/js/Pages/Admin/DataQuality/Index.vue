    ...
    <div class="py-12">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
              ...
              <div v-for="similar in client.similar_clients" :key="similar.id"
                ...
                <div class="flex items-center justify-between mb-2">
                  <div class="flex items-center space-x-3">
                    <p class="font-medium text-slate-900">{{ similar.display_name }}</p>
                  ...
                  <div class="text-sm">
                    <span class="text-slate-500">{{ t('Similarity') }}:</span>
                    <span class="ml-1 font-medium text-slate-700">{{ similar.similarity_scores.total }}%</span>
                  </div>
                </div>
        ...
        <div v-if="activeTab === 'properties'" class="space-y-4">
          <div v-for="property in duplicate_properties" :key="property.id" class="bg-white shadow-sm ring-1 ring-slate-200 rounded-lg p-4">
            <div class="border-b border-slate-200 pb-4 mb-4">
              <div class="flex items-center justify-between">
                <h3 class="text-lg font-medium text-slate-900">{{ property.name }}</h3>
                <div class="flex items-center space-x-2">
                  <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium bg-indigo-50 text-indigo-700">
                    {{ t('Total Instances') }}: {{ property.total_instances }}
                  </span>
                  <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium bg-emerald-50 text-emerald-700">
                ...
                <div class="space-y-2">
                  <div v-for="rent in property.rent_history" :key="rent.id"
                    ...
                    <div>
                      ...
                      <span class="text-slate-500"> - {{ rent.tenant_name }}</span>
            ...
            <div class="space-y-3">
              ...
              <div v-for="similar in property.similar_properties" :key="similar.id"
                ...
                <div class="flex items-center justify-between mb-2">
                  <div>
                    <div class="flex items-center space-x-2">
                      <p class="font-medium text-slate-900">{{ similar.name }}</p>
                    ...
                    <p class="text-sm text-slate-500">{{ similar.address }}</p>
                    <p class="text-sm text-slate-500">{{ t('Owner') }}: {{ similar.owner_name }}</p>
                  </div>
                  <div class="text-sm space-y-1">
                    <p>
                      <span class="text-slate-500">{{ t('Name Match') }}:</span>
                      <span class="ml-1 font-medium text-slate-700">{{ similar.similarity_scores.name }}%</span>
                    </p>
                    <p>
                      <span class="text-slate-500">{{ t('Address Match') }}:</span>
                      <span class="ml-1 font-medium text-slate-700">{{ similar.similarity_scores.address }}%</span>
                    </p>
                    <p>
                      <span class="text-slate-500">{{ t('Total Match') }}:</span>
                      <span class="ml-1 font-medium text-slate-700">{{ similar.similarity_scores.total }}%</span>
                    </p>
                  </div>
                </div>
        ...
        <div v-if="activeTab === 'rents'" class="space-y-4">
          <div v-for="rent in inconsistent_rents" :key="rent.id" class="bg-white shadow-sm ring-1 ring-slate-200 rounded-lg p-4">
            <div class="border-b border-slate-200 pb-4 mb-4">
              <div class="flex items-center justify-between">
                ...
                <span :class="{
                  'bg-rose-100 text-rose-800': rent.issue_type === 'overlap',
                  'bg-amber-100 text-amber-800': rent.issue_type === 'gap',
