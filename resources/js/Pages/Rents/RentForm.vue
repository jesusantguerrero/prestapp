...
watch(() => rentForm.property, (property) => {
  if (property) {
      ...
      <main class="w-full p-5 bg-white rounded-md text-body-1">
        ...
        <article class="w-full space-y-4">
          <FormSection title="Datos de deposito">
              <AtField label="Deposito" class="w-full">
                <AtInput :number-format="true" v-model="rentForm.deposit" rounded>
                  <template #suffix>
                    <button class="w-32 px-2 transition" :class="[rentForm.is_deposit_received ? 'bg-success text-white font-bold' : 'bg-base-lvl-2 text-body-1']" @click="rentForm.is_deposit_received = !rentForm.is_deposit_received">
          ...
          <FormSection title="Moras" section-class="space-y-4">
            <section class="mt-4">
              <AppButton> Aplicar Moras</AppButton>
            </section>
