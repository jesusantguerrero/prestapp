  ...
  <div>
    <div class="card-body section-actions" v-if="showActionBar">
      <div class="app-search__container">
        ...
        >
          <template v-slot:date v-if="isTemplate('week')">
            <div class="d-flex">
              <button class="btn btn-primary" @click="$refs.Schedule.gotoToday()">
      ...
      <div class="action-buttons row d-flex justify-content-end">
        ...
        <el-dropdown split-button type="primary" class="col-auto" v-if="showActions">
          Acciones
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command="edit" data>Exportar PDF</el-dropdown-item>
    ...
    <div>
      ...
      >
        <template v-for="day in week" v-slot:[getISODate(day)]>
          <div :key="day.toISOString()">
            <div v-for="(item, index) in getForDate(day)" :key="`${item.fecha}-${index}`">
              <show-card :item="item" />
            </div>
          </div>
        </template>
      ...
      >
        ...
        <slot slot="append" slot-scope="props" v-bind="{ ...props }" name="append">
        </slot>
      </ic-table>
    </div>

    <div v-if="config.pagination" class="pagination-container d-flex justify-content-end">
      <el-pagination
        background
...
export default {
  ...
  created() {
    this.cols.forEach((col, index) => {
      this.cols[index].sortable =
        this.cols[index].sortable === true ? "custom" : this.cols[index].sortable;
    });
  ...
  computed: {
    ...
    filterParams() {
      ...
      if (this.config.urlParams && this.config.urlParams.filter) {
        Object.entries(this.config.urlParams.filter).forEach(([name, value]) => {
  ...
  methods: {
    ...
    getTableData() {
      ...
      this.$http.get(`${this.config.resourceUrl}?${this.params}`).then(({ data }) => {
        if (data) {
