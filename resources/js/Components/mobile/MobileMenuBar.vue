<template>
  <div
    class="flex w-screen overflow z-50 fixed left-0 -bottom-1 h-14 bg-gray-900 rounded-t-2xl lg:hidden"
  >
    <div v-for="item in menu" class="w-full h-full">
      <MobileMenuItem :item="item" v-if="item.url || item.to" />
      <div v-else class="relative mx-auto">
        <ButtonCircle
          class="mx-auto -translate-y-7"
